...
class dbConvert {
   ...
   constructor(packName = "kctg-fade") {
...
Options:
  --pack <name>          Specify pack name
  --file <path>          Specify custom .db file path (extract only)
  --help                 Show help

Examples:
  node "scripts/build/dbConvert.mjs" extract --pack kctg-fade
  node "scripts/build/dbConvert.mjs" extract --file ./packs/kctg-fade.db
  node "scripts/build/dbConvert.mjs" compile --pack kctg-fade
  node "scripts/build/dbConvert.mjs" help
   ...
   async handleExtract() {
      ...
      if (customFile) {
         const converter = new dbConvert("kctg-fade"); // Default pack name for custom file
