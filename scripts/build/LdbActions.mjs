 ...
 */
class LdbActions {
   static AVAILABLE_PACKS = ["kctg-fade", "kctg-fade-armaments"];
   ...
   async dump() {
      ...
      for await (const [key, value] of db.iterator()) {
         try {
            // if (key =="!kctg-fade!yuscgQo8thsyp6HP") console.debug(key, value);
...
Examples:
  ...
  node scripts/build/LdbActions.mjs dump --pack kctg-fade            # Dump specific pack
  node scripts/build/LdbActions.mjs restore                       # Restore all packs
  node scripts/build/LdbActions.mjs restore --pack kctg-fade          # Restore specific pack
  node scripts/build/LdbActions.mjs restore --pack kctg-fade --backup
  node scripts/build/LdbActions.mjs checkpack --pack kctg-fade
